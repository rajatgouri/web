...
export class AuthService {
  ...
  generateAutoLoginToken(id:string, type:any): Promise<any> {
    return this.restangular.all('users/generatetoken').customPOST({'_id':id, 'type':type}).toPromise();
  }

  getUserByToken(token:string) {
    return this.restangular.all('user/token').customPOST({'token': token }).toPromise();
  }
